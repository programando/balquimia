<?php
  ...
  class EmpresaController extends Controller
  {
    ...
    public function enviar_correo() {
      ...
      if ( $Email_Ok == false){
        $Texto_Respuesta =$Texto_Respuesta .'\nDirección de correo no tiene un formato válido.';
      }
      if ( strlen($telefono) == 0 ) {
        $Texto_Respuesta =$Texto_Respuesta .'\nNúmero de teléfono no válido.';
      }
      if ( strlen($linea) == 0 || $linea == 'Seleccione...') {
        $Texto_Respuesta =$Texto_Respuesta .'\nIndíquenos cuál es su línea de interés.';
      }

      if ( strlen($tipo_mensaje) == 0 || $tipo_mensaje == 'Seleccione...') {
        $Texto_Respuesta =$Texto_Respuesta .'\nIndíquenos cuál es el tipo de mensaje.';
      }

      if ( strlen($mensaje) == 0 || $mensaje == 'Seleccione...') {
        $Texto_Respuesta =$Texto_Respuesta .'\nIndíquenos cuál es el mensaje que desea enviar.';
      }
